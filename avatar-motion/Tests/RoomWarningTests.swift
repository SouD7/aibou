...
func testRoomWarnings() throws {
    ...
    try expect(scene.hoveredComponent?.id == "fan-1", "warning badge is hoverable outside the original component")
    scene.selectRoom(at: anchor)
    try expect(scene.selectedComponent?.id == "fan-1", "badge opens matching component")
    ...
    try expect(scene.selectedComponent?.id == "fan-1", "retained badge works after returning to room")
