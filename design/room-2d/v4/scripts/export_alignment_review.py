...
import json
B=Path(__file__).resolve().parents[1];S=B.parent/'v3';m=json.loads((B/'manifest.json').read_text());fx=json.loads((B/'effects/manifest.json').read_text());p=B/'previews'
...
for i,s in enumerate(m['scenes']):
 im=Image.open(B/s['preview']).convert('RGB').resize((820,462));x=(i%2)*836+8;y=(i//2)*515+43;o.paste(im,(x,y));d.text((x+8,y-33),s['id'].upper()+' / v4',font=font,fill=(210,234,248))
o.save(p/'cross-camera-v4.jpg',quality=94)
# Compare the changed silhouette and arrangement.
o=Image.new('RGB',(1672,515),(20,26,36));d=ImageDraw.Draw(o)
for i,(base,title) in enumerate([(S,'BEFORE / v3'),(B,'AFTER / v4')]):
