"""Validate v4 image and animation contracts without requiring a browser."""
from pathlib import Path
import json,zipfile,hashlib,xml.etree.ElementTree as ET,io
import numpy as np
from PIL import Image
B=Path(__file__).resolve().parents[1];m=json.loads((B/'manifest.json').read_text());fx=json.loads((B/'effects/manifest.json').read_text());report={'version':4,'canvas':[1672,941],'scenes':[],'browserQA':'Not run: local URL rejected by browser tool policy.'}
def rgba(path):
