...
for scene in m['scenes']:
    ...
    base=Image.open(B/scene['preview']).convert('RGBA'); frames=[]
    for i in range(12):
        frame=base.copy()
        frame.alpha_composite(Image.open(B/'effects'/fx['compute']['frames'][i]))
        for fan in fx['fans']:frame.alpha_composite(Image.open(B/'effects'/fan['frames'][i]))
...
print('Created 4 motion WebP, 4 layer atlases, 1 contact sheet')
