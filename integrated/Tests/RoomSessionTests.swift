...
struct RoomSessionTests {
    static func main() {
        ...
        precondition(session.nextPoseAt == origin.addingTimeInterval(10))
        precondition(!session.shouldChoosePose(now: origin.addingTimeInterval(9.999)))
        precondition(session.shouldChoosePose(now: origin.addingTimeInterval(10)))
        precondition(!session.shouldChoosePose(now: origin.addingTimeInterval(10)))
        precondition(!session.shouldChoosePose(now: origin.addingTimeInterval(19.999)))
        precondition(session.shouldChoosePose(now: origin.addingTimeInterval(20)))
        ...
        precondition(!session.shouldChoosePose(now: origin.addingTimeInterval(909.999)))
        precondition(session.shouldChoosePose(now: origin.addingTimeInterval(910)))
        session.enterDemo()
        ...
        precondition(!session.shouldChoosePose(now: origin.addingTimeInterval(1009.999)))
        precondition(session.shouldChoosePose(now: origin.addingTimeInterval(1010)))
        ...
        print("RoomSessionTests: OK (10-second timer, consultation, demo, acknowledgement, relaunch)")
    }
}
